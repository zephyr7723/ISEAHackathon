...
mod tests {
    ...
    type F = <Bn256Engine as Engine>::Scalar;

    // Helpers to build consistent test fixtures

    fn make_fixture(
        dob: u64, t_val: u64,
        sig_s1: u64, sig_s2: u64,
        com_cnat: u64,
        age_lo: u64, age_hi: u64,
    ) -> (PsDobCircuit<F>, Vec<F>) {
        let com_odob = F::from(0xdeadbeef_u64) * F::from(dob);
        let t        = F::from(t_val);
        let com_cdob = com_odob * t;

        let circuit = PsDobCircuit::new(dob, com_odob, t, F::from(age_lo), F::from(age_hi));
        let pub_in  = compute_public_inputs::<F>(
            F::from(sig_s1), F::from(sig_s2),
            com_cdob, F::from(com_cnat),
            F::from(age_lo), F::from(age_hi),
            F::ZERO,
        );
        (circuit, pub_in.to_vec())
    }

    fn alloc_z_in(cs: &mut TestConstraintSystem<F>, pub_in: &[F]) -> Vec<AllocatedNum<F>> {
        let names = ["sig_s1","sig_s2","com_cdob","com_cnat","age_lo","age_hi","country"];
        pub_in.iter().zip(names.iter()).map(|(&v, &name)| {
            AllocatedNum::alloc(cs.namespace(|| name), || Ok(v)).unwrap()
        }).collect()
    }
    ...
    #[test]
    fn test_valid_outputs_all_four() {
        let (circuit, pub_in) = make_fixture(19900115, 7, 1234, 5678, 9999, 18, 65);
        let mut cs = TestConstraintSystem::<F>::new();
        let z_in   = alloc_z_in(&mut cs, &pub_in);
        let z_out  = circuit.synthesize(&mut cs, None, &z_in).unwrap().1;

        assert!(cs.is_satisfied(), "failed: {:?}", cs.which_is_unsatisfied());
        assert_eq!(z_out.len(), 4);

        // All four outputs equal the corresponding z_in values
        for (i, (o, z)) in z_out.iter().zip(z_in.iter()).enumerate() {
            assert_eq!(
                o.get_value().unwrap(), z.get_value().unwrap(),
                "z_out[{i}] should equal z_in[{i}]"
            );
        }
    ...
    #[test]
    fn test_sig_s1_zero_outputs_default() {
        let (circuit, mut pub_in) = make_fixture(19900115, 7, 1234, 5678, 9999, 18, 65);
        pub_in[0] = F::ZERO;  // sig_s1 = 0 (garbage)
        let mut cs = TestConstraintSystem::<F>::new();
        let z_in   = alloc_z_in(&mut cs, &pub_in);
        let z_out  = circuit.synthesize(&mut cs, None, &z_in).unwrap().1;
        assert!(cs.is_satisfied());
        for o in &z_out {
            assert_eq!(o.get_value().unwrap(), F::ZERO);
        }
    }

    #[test]
    fn test_sig_s2_zero_outputs_default() {
        let (circuit, mut pub_in) = make_fixture(19900115, 7, 1234, 5678, 9999, 18, 65);
        pub_in[1] = F::ZERO;  // sig_s2 = 0 (garbage)
        let mut cs = TestConstraintSystem::<F>::new();
        let z_in   = alloc_z_in(&mut cs, &pub_in);
        let z_out  = circuit.synthesize(&mut cs, None, &z_in).unwrap().1;
        assert!(cs.is_satisfied());
        assert_eq!(z_out[0].get_value().unwrap(), F::ZERO);
    }

    #[test]
    fn test_com_cdob_zero_outputs_default() {
        // com_cdob = 0 is a garbage commitment
        // We must build a consistent fixture where com_odob * t = 0
        // which happens when t = 0 (but t=0 is also invalid -- use directly)
        let mut cs = TestConstraintSystem::<F>::new();
        let com_odob = F::from(12345u64);
        let t        = F::ZERO;             // t=0 -> com_cdob = 0 (garbage)
        let com_cdob = com_odob * t;        // = 0
        let circuit  = PsDobCircuit::new(19900115, com_odob, t, F::from(18u64), F::from(65u64));
        let pub_in   = compute_public_inputs::<F>(
            F::from(1234u64), F::from(5678u64),
            com_cdob, F::from(9999u64),
            F::from(18u64), F::from(65u64), F::ZERO,
        );
        let z_in = alloc_z_in(&mut cs, &pub_in);
        let z_out = circuit.synthesize(&mut cs, None, &z_in).unwrap().1;
        assert!(cs.is_satisfied());
    ...
    #[test]
    fn test_wrong_t_fails_randomisation() {
        let (_, pub_in) = make_fixture(19900115, 7, 1234, 5678, 9999, 18, 65);
        let com_odob    = F::from(0xdeadbeef_u64) * F::from(19900115u64);
        // Witness claims t=999 but com_cdob in z_in was built with t=7
        let circuit = PsDobCircuit::new(19900115, com_odob, F::from(999u64), F::from(18u64), F::from(65u64));

        let mut cs = TestConstraintSystem::<F>::new();
        let z_in   = alloc_z_in(&mut cs, &pub_in);
        let _ = circuit.synthesize(&mut cs, None, &z_in).ok();
        assert!(!cs.is_satisfied(), "wrong t must fail randomisation constraint");
        assert_eq!(
    ...
    #[test]
    fn test_age_out_of_range_outputs_default() {
        // dob = 2010 -> age 15, age_lo = 18
        let (circuit, pub_in) = make_fixture(20100101, 7, 1234, 5678, 9999, 18, 65);
        let mut cs = TestConstraintSystem::<F>::new();
        let z_in   = alloc_z_in(&mut cs, &pub_in);
        let z_out  = circuit.synthesize(&mut cs, None, &z_in).unwrap().1;
        assert!(cs.is_satisfied());
    ...
    #[test]
    fn test_age_at_lower_bound_passes() {
        let dob = (CURRENT_YEAR - 18) * YEAR_DIVISOR + 101;
        let (circuit, pub_in) = make_fixture(dob, 7, 1234, 5678, 9999, 18, 65);
        let mut cs = TestConstraintSystem::<F>::new();
        let z_in   = alloc_z_in(&mut cs, &pub_in);
        let z_out  = circuit.synthesize(&mut cs, None, &z_in).unwrap().1;
        assert!(cs.is_satisfied());
        assert_eq!(z_out[0].get_value().unwrap(), F::from(1234u64));
    }

    #[test]
    fn test_age_at_upper_bound_passes() {
        let dob = (CURRENT_YEAR - 65) * YEAR_DIVISOR + 101;
        let (circuit, pub_in) = make_fixture(dob, 7, 1234, 5678, 9999, 18, 65);
        let mut cs = TestConstraintSystem::<F>::new();
        let z_in   = alloc_z_in(&mut cs, &pub_in);
        let z_out  = circuit.synthesize(&mut cs, None, &z_in).unwrap().1;
    ...
    // ── com_cnat is passed through unchanged ──────────────────────────────

    #[test]
    fn test_com_cnat_passes_through_unchanged() {
        let (circuit, pub_in) = make_fixture(19900115, 7, 1234, 5678, 9999, 18, 65);
        let mut cs = TestConstraintSystem::<F>::new();
        let z_in   = alloc_z_in(&mut cs, &pub_in);
        let z_out  = circuit.synthesize(&mut cs, None, &z_in).unwrap().1;
        assert!(cs.is_satisfied());
        // z_out[3] must equal z_in[3] (com_cnat)
        assert_eq!(
            z_out[3].get_value().unwrap(),
            z_in[3].get_value().unwrap(),
        );
    }
}
