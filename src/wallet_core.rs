...
pub fn prove_credential(
    ...
    let com_dob_f1 = ark_fr_to_halo(&rand_age_opt.as_ref().unwrap().rk)*ark_fr_to_halo(&m1);
    let com_nat_f1 = ark_fr_to_halo(&rand_nat_opt.as_ref().unwrap().rk)*ark_fr_to_halo(&m2);
    ...
    let t_mul1 = t_halo * ark_fr_to_halo(&rand_age_opt.as_ref().unwrap().rk);
    let t_mul2 = t_halo * ark_fr_to_halo(&rand_nat_opt.as_ref().unwrap().rk);
    ...
    let nizk_nat = rerandomise_proof(&pk, &(proof_nat_opt.unwrap()),&(rand_nat_opt.unwrap()),t,m1,m2).ok();
    let nizk_dob = rerandomise_proof(&pk, &(proof_age_opt.unwrap()),&(rand_age_opt.unwrap()),t,m1,m2).ok();
